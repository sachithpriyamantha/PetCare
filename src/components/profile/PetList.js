...
const PetList = () => {
  ...
  const fetchPets = async () => {
    try {
      setLoading(true);
      const response = await axios.get("https://pet-care-api-ten.vercel.app/api/pets", {
        withCredentials: true,
      });

      // Show all pets instead of filtering by user
  ...
  const handleDeletePet = async (e, petId, petName, petOwner) => {
    e.preventDefault();
    e.stopPropagation();

    // Only allow deletion if the current user is the owner
    if (!user || user.username !== petOwner) {
      Swal.fire({
        title: "Access Denied",
        text: "You can only delete your own pets.",
        icon: "error",
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`https://pet-care-api-ten.vercel.app/api/pets/${petId}`, {
          withCredentials: true,
        });
  ...
  if (!user) {
    return (
      <div className="text-center p-8">
        <p className="text-amber-600 mb-4">Please log in to view pets</p>
      ...
      {pets.length === 0 ? (
        <div className="text-center p-12 bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <p className="mt-2 text-gray-500">
            Be the first to add a pet to the platform.
          </p>
          <div className="mt-6">
            <Link
              to="/pets/register"
            ...
            >
              <Link to={`/pets/${pet._id}`} className="block h-full">
                <div className="h-48 bg-gray-200 relative">
                  {pet.image ? (
                    <img
                      src={`https://pet-care-api-ten.vercel.app/${pet.image}`}
                      alt={pet.name}
              ...
              {/* Delete button - only show for pet owner */}
              {user && user.username === pet.ownerName && (
                <button
                  onClick={(e) => handleDeletePet(e, pet._id, pet.name, pet.ownerName)}
