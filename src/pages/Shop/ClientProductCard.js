...
const ClientProductCard = ({ product, onAddToCart }) => {
  ...
  // Convert price to LKR
  const priceInLKR = (price * 300).toFixed(2);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:shadow-lg hover:-translate-y-1 flex flex-col h-full">
      ...
      <div className="p-4 flex flex-col flex-grow">
        <h2 className="font-bold text-lg text-gray-800 mb-2 line-clamp-1">{name}</h2>
        <p className="text-gray-600 text-sm mb-4 line-clamp-2 flex-grow">{description}</p>
        
        <div className="flex justify-between items-center">
          <span className="text-lg font-semibold text-indigo-700">LKR {priceInLKR}</span>
          ...
          <div className="flex space-x-2">
            ...
            <button
              onClick={onAddToCart}
              disabled={stock <= 0}
              className={`inline-flex items-center px-2 py-1 border border-transparent rounded text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors duration-150
                ${stock <= 0 
